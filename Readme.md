...
Никакой разницы.
Попробуем увеличить частоту ещё раз.
...
Итак, результат увеличился в 14 раз.
Попробуем это оптимизировать.
На гистограмме видно, что она довольно сильно растянута.
И можно предположить, что это связанно с тем, что система не успевает это всё вычитывать вовремя.
Чтобы помочь приложению, сменим политику для шедулера на [SCHED_FIFO](https://lwn.net/Articles/296419/).
Это можно сделать при помощи утилиты `chrt`.
Которую можно использовать при помощи агрумента при сборке `SCHEDFIFO`.
Пробуем:
```
make plot MSGLEN=1048576 FREQ=10000 SCHEDFIFO=1
```

![f=10000 size=1MB schedfifo](images/plot_schedfifo_s1M_f1e4.png)

Результат улучшился в 3 раза.
Т.е. как одно из решений проблемы имеем смену политики шедулера.

Можно ещё увеличить стабильность всего этого, выделив себе отдельные CPU в системе.
Для этого есть аргумент `SCHEDCPU`. Который лучше всего исользовать в паре с параметром ядра `isolcpus`.

Сначала, надо подредактировать `cmdline` загрузчика и вписать туда, например: `isolcpus=3,4`.
Потом запустим наше приложение на этих ядрах:
```
make plot MSGLEN=1048576 SCHEDFIFO=1 SCHEDCPU=3,4
```
_Примечание: поскольку приложение тестовое и сырое, оно ведёт себя нестабильно на некоторых системах при отправке больших пакетов на высокой частоте. Поэтому ниже используется f=100._

![f=100 size=1MB schedfifo schedcpu](images/plot_schedfifo_schedcpu_s1M_f1e2.png)

Получили очень хороший результат.


### Nagle's algorithm
А что если попытаться отключить ожидание мелких пакетов?

```
make plot NODELAY=1
```

![f=100 size=1KB nodelay](images/plot_nodelay_f1e2.png)

Разница небольшая, по сравнению с тем же кейсом в самом начале.

А что если частоту поднять повыше?
```
make plot NODELAY=1 FREQ=1000000000
```
![f=1000000000 size=1KB nodelay](images/plot_nodelay_f1e9.png)

А вот тут уже наблюдаем странное.
Частоту увеличили, но при этом показатели стали лучше.

Всё дело в том, что приложению изначально приходится часто переключать контекст между пользовательским и пространством ядра для проверки буферов.
И на высокой частоте - это делать приходится гораздо реже.
Из-за чего и получаем подобный эффект.

<!--Чтобы продемонстрировать что это работает, возьмём пакеты ещё меньше.
```
make plot NODELAY MSGLEN=32
```
![f=100 size=32 nodelay](images/plot_nodelay_s32_f1e2.png)

```
make plot NODELAY MSGLEN=32 FREQ=1000000000
```
![f=1000000000 size=32 nodelay](images/plot_nodelay_s32_f1e9.png)
-->
...
Можно переключать при помощи параметра `NOBLOCK`
...
Можно переключать при помощи параметра `NODELAY`
...
Включается при помощи `setsockopt()`.
В коде есть заготовка для реализации через параметр `SCHEDCPU`.
Но недоделано на текущий момент и закоментированно.
...
Опцию ядра можно включить через тот же GRUB.
А выбор CPU в приложении осуществлять при помощи `SCHEDCPU`, указывая ядра через запятую.
...
Сейчас можно включить при помощи аргумента `SCHEDFIFO`.
Достаточно его определить, чтобы он был.
Например:
```
make plot SCHEDFIFO=1
```
Но надо учитывать, что для запуска приложения `chrt` требуются права пользователя `root`.
